...
import sigup_web.entidades.Modulo;
import sigup_web.util.GenericPersistence;

@Path("/modulo")
public class ModuloWebService{
	...
	public Response inserir(Modulo obj){
		try{
	...
	@PUT
	@Consumes("application/json")
	public Response alterar(Modulo obj){
		try{
	...
	public List<Modulo> listarTodos(){
		try{
			List<Modulo>listaModulo = persistence.listar(Modulo.class);
			return listaModulo;
		}catch(Exception e){
			e.printStackTrace();
			throw new WebApplicationException(500);
	...
	@Path("/excluir/{idModulo}")
	@DELETE
	public Response excluir(@PathParam("idModulo") Integer idModulo){
		try{
			Modulo obj = (Modulo) persistence.buscarPorId(Modulo.class, idModulo);
